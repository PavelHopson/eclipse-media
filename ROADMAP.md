...
- Если публичный видеопоток отвечает HTTP 403 после успешной проверки ссылки, Media один раз
  повторяет загрузку через совместимый HLS-поток не выше выбранного разрешения.
- Повтор ограничен одним безопасным селектором и не использует cookies, логины, пароли или raw CLI;
  источники с авторизацией по-прежнему завершаются явной ошибкой без обхода ограничений.
- Добавлены регрессионные тесты выбора разрешения, запрета повторов для авторизации и успешного
  восстановления после первого HTTP 403.
