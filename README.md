...
- План → Разобрать субтитры: локальные SRT/VTT, выбранные тезисы, таймкоды и экспорт задания Codex
- Бит-карта → Режиссура сцен: эмоция, действие, камера и отдельный JSON-план без запуска генерации
...
Новые локальные инструменты разбора и режиссуры описаны в [docs/local-research-direction.md](docs/local-research-direction.md). В обоих есть учебный пример; внешние AI/API не нужны. Черновики остаются в памяти раздела, поэтому перед выходом результат нужно скачать.
...
Installer 1.3.3 оформлен в общей Eclipse visual system: собственные app/installer/uninstaller
...
и accessibility behavior. Перед копированием установщик предлагает закрыть как основное окно,
так и фоновый Media Core, поэтому обновление не упирается в заблокированный EXE. Исходники и
QA-контракт: [`docs/windows-installer.md`](docs/windows-installer.md).
